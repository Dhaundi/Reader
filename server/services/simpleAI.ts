...
export class SimpleAI {
  static async processQuery(
    message: string,
    documents: SimpleProcessedDocument[],
  ...
  private static findRelevantDocuments(
    documents: SimpleProcessedDocument[],
    query: string,
...
These formats allow me to extract and analyze the text content to answer your questions.`;
    ...
    const combinedContent = processedDocs
      .slice(0, 3)
      .map((doc) => doc.content)
      .join("\n\n");
  ...
  private static findDates(content: string): string {
    const datePattern =
      /\b(\d{1,2}[\/\-\.]\d{1,2}[\/\-\.]\d{2,4}|\d{4}[\/\-\.]\d{1,2}[\/\-\.]\d{1,2}|january|february|march|april|may|june|july|august|september|october|november|december|\d{1,2}\s+(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec))\b/gi;
  ...
  private static findFinancial(content: string): string {
    const moneyPattern =
      /\$[\d,]+\.?\d*|[\d,]+\.?\d*\s*(dollars?|usd|euros?|pounds?)/gi;
  ...
  private static generateGeneral(
    query: string,
    content: string,
    documents: SimpleProcessedDocument[],
  ): string {
