...
export class GroqAI {
  ...
  async processQuery(query: string, documents: ProcessedDocument[]): Promise<GroqResponse> {
    ...
    try {
      ...
      const response = await this.generateGroqResponse(query, context, queryType, documents.length);
      ...
      // Fallback response
      return {
        answer: "I apologize, but I encountered an error while processing your question. Please try rephrasing your query or check if the documents contain the information you're looking for.",
        confidence: 0.1,
  ...
  private analyzeQueryType(query: string): string {
    ...
    if (/\b(summarize|summary|overview|main points|key points)\b/.test(lowerQuery)) {
      return 'summary';
      ...
      return 'temporal';
    } else if (/\b(cost|price|amount|money|budget|financial)\b/.test(lowerQuery)) {
      return 'financial';
...
- If asked about something not in the documents, explain what information IS available instead`;
    ...
    return basePrompt + (typeSpecificPrompts[queryType as keyof typeof typeSpecificPrompts] || typeSpecificPrompts.general);
  }

  private calculateConfidence(chunks: VectorDocument[], response: string): number {
    ...
    if (chunks.length > 0) {
      const avgSimilarity = chunks.reduce((sum, chunk) => 
        sum + ((chunk as any).similarity || 0), 0) / chunks.length;
      confidence += avgSimilarity * 0.15;
    }
    
    // Reduce confidence if response indicates uncertainty
    if (response.toLowerCase().includes("i don't have information") ||
