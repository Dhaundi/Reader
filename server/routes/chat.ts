...
export const getDocumentSummary: RequestHandler = (req, res) => {
  try {
    ...
    const userDocuments = simpleDocumentStore.getUserDocuments(
      userId as string,
    );

    const summary = {
      totalDocuments: userDocuments.length,
      totalWordCount: userDocuments.reduce(
        (sum, doc) => sum + doc.metadata.wordCount,
        0,
      ),
      totalChunks: userDocuments.length, // Simple processor doesn't chunk documents
      documentTypes: userDocuments.reduce(
        (types, doc) => {
          const type = doc.type.includes("pdf")
            ? "PDF"
            : doc.type.includes("word")
              ? "DOCX"
              : doc.type.includes("html")
                ? "HTML"
                : doc.type.includes("message")
                  ? "Email"
                  : "Other";
